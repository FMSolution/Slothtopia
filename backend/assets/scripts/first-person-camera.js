...
CharacterController.prototype.initialize = function () {
    this.slothPlayer = this.app.root.findByName('playerSloth');
    ...
    this.onGround = true;
    this.jumping = false;
    this.slothPlayer.anim.assignAnimation('walk', this.app.assets.find('walk.glb').resource);
    this.slothPlayer.anim.assignAnimation('idle', this.app.assets.find('idle.glb').resource);
    this.slothPlayer.anim.assignAnimation('jump', this.app.assets.find('jump.glb').resource);
};


CharacterController.prototype.move = function (direction) {
    // console.log('sloth on ground ? =',this.onGround);
    if (this.onGround && !this.jumping) {
        var tmp = new pc.Vec3();
        var length = direction.length();
        if (length > 0) {
            // calculate new forward vec parallel to the current ground surface
            tmp = tmp.cross(this.groundNormal, direction).cross(tmp, this.groundNormal);
            tmp = tmp.normalize().scale(length * this.speed);
...
CharacterController.prototype.jump = function () {
    if (this.onGround && !this.jumping) {
        this.slothPlayer.anim.baseLayer.transition("jump");
...
CharacterController.prototype.postUpdate = function () {
    var currentAnimation = this.entity.anim.baseLayer.activeState;
    var currentVelocity = this.entity.rigidbody.linearVelocity;
    currentVelocity.x = parseInt(currentVelocity.x);
    currentVelocity.z = parseInt(currentVelocity.z);
    if(Math.abs(parseInt(currentVelocity.x)) >= 8 || Math.abs(parseInt(currentVelocity.z)) >= 8){
        if(currentAnimation != "walk"){
            this.entity.anim.baseLayer.transition("walk",0.1,0.5);
        }
    }else{
        if(currentAnimation != 'idle' && !this.jumping){
            this.entity.anim.baseLayer.transition("idle",0.1,0.5);
        }
    }
}
...
FirstPersonCamera.prototype.initialize = function () {
    ...
    app.on('firstperson:flip', function() {
        if(!this.flipped){
            this.camera.translate(-35,0,0);
            this.flipped = true;
        }else{
            this.camera.translate(35,0,0);
            this.flipped = false;
...
KeyboardInput.prototype.initialize = function () {
    ...
    var keyDown = function (e) {
        if (!e.repeat) {
            ...
            // this.slothPlayer.anim.baseLayer.transition("walk", 0.01);
