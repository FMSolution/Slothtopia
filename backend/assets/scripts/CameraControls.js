...
CameraMovement.attributes.add('isMinting', { type: 'boolean', default: false, description: 'is the player minting'});
...
CameraMovement.prototype.checkMint = function () {
    var pos = this.entity.parent.getPosition();
    var x = parseInt(pos.x);
    var z = parseInt(pos.z);
    if(x >= 20 && x <= 30 && z >= 24 && z <= 26){
        this.isMinting = true;
        window.mint();
    }
}
