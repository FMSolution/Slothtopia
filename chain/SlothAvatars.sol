...
contract SlothAvatar is Ownable, ERC165, IERC721, IERC721Metadata {
    ...
    constructor(string memory _url, string memory _extention, string memory _gURL) {
        baseURL = _url;
        baseExtention = _extention;
        gameURL = _gURL;
    }

    function changeURLParams(string memory _nURL, string memory _nBaseExt, string memory _ngURL) external onlyOwner {
        baseURL = _nURL;
    ...
    function gameURI(uint256 tokenID) public view returns(string memory){
        return(string(abi.encodePacked(gameURL, SlothHelper.toString(tokenID), baseExtention)));
    }
